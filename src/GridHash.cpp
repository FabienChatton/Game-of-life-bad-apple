...
void GridHash::RandomGrid(int row, int col)
{
	srand(time(0));
	for (int i = 0; i < row; i++) {
		for (int j = 0; j < col; j++) {
...
std::unordered_map<long int, Pos> GridHash::GetPosToCheck()
{
...
std::vector<Cell> GridHash::GetAlivesCells()
{
...
std::optional<Cell> GridHash::GetCellAt(int x, int y)
{
